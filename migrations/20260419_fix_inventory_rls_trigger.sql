-- Migration: Fix RLS policy violation on inventory_products when updating deliveries
-- Issue: The trigger function update_inventory_from_delivery() violates RLS policies on inventory_products
-- Solution: Make the function run with SECURITY DEFINER to bypass RLS
...
'Automatically updates inventory_products when a delivery note status is set to "delivered". Adds quantities to existing products or creates new ones. Runs with SECURITY DEFINER to bypass RLS policies.';
