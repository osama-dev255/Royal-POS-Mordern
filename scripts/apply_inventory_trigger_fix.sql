-- Script to apply the inventory trigger fix

-- This script applies the fix to remove status dependency from inventory updates

-- First, check current function definition
SELECT pg_get_functiondef('update_inventory_from_delivery'::regproc) as current_definition;

-- Apply the fixed function (this will replace the existing one)
...
SELECT 
  CASE 
    WHEN pg_get_functiondef('update_inventory_from_delivery'::regproc) ~ 'status.*!=.*''delivered''' THEN 'STATUS CONDITION STILL PRESENT'
    ELSE 'STATUS CONDITION REMOVED - SUCCESS'
